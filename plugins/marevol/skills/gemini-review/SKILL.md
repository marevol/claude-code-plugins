---
name: gemini-review
description: Review gate using Gemini CLI (read-only) to iterate review and fix until convergence
user-invocable: true
...
Iterative review gate: Gemini audits (read-only), Claude Code fixes, re-review until `ok: true`.
...
2. **Run architecture review** (medium and large only)
   ...
   ```
   - Execute `gemini -p "<PROMPT>" --sandbox -o json` and wait for completion
...
6. **Handle errors**
   - If `gemini` fails: retry once (for timeout, halve the file count)
...
Instruct Gemini to output exactly one JSON object. Append this schema to the end of every review prompt where `[APPEND OUTPUT SCHEMA]` appears.
